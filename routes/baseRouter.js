...
router.get("/next-block", asyncHandler(async (req, res, next) => {
	...
	// For PoS chains, getBlockTemplate returns null
	if (!blockTemplate) {
		res.locals.blockTemplate = null;
		res.locals.minFeeRate = 0;
		res.locals.maxFeeRate = 0;
		res.locals.medianFeeRate = 0;
...
router.get("/block-height/:blockHeight", asyncHandler(async (req, res, next) => {
	try {
		...
		if (global.specialBlocks && global.specialBlocks[res.locals.result.getblock.hash]) {
			...
			res.locals.metaTitle = `Blackcoin Block #${blockHeight.toLocaleString()}`;
			res.locals.metaDesc = "";
		}
...
router.get("/block/:blockHash", asyncHandler(async (req, res, next) => {
	try {
		...
		if (global.specialBlocks && global.specialBlocks[res.locals.result.getblock.hash]) {
			...
			res.locals.metaTitle = `Blackcoin Block ${utils.ellipsizeMiddle(res.locals.result.getblock.hash, 16)}`;
			res.locals.metaDesc = "";
		}
...
router.get("/tx/:transactionId", asyncHandler(async (req, res, next) => {
	try {
		...
		if (global.specialTransactions && global.specialTransactions[txid]) {
			...
			res.locals.metaTitle = `Blackcoin Transaction ${utils.ellipsizeMiddle(txid, 16)}`;
			res.locals.metaDesc = "";
		}
...
router.get("/address/:address", asyncHandler(async (req, res, next) => {
	...
	try {
		...
		res.locals.metaTitle = `Blackcoin Address ${address}`;
